...
installComplete, building, progress, sshState, sshPort, vncPort, replica (Linux: "" / none / stopped / running, the default nested replica), replicas (Linux: every nested replica as `[{name, state, vnc, desktop}]`), replicaAuto, qemu (Linux: "" / none / stock / building / patched, the QEMU the replicas run on, per the guest agent), ramMb, hddGb, cpuCores,
