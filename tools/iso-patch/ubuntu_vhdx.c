...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "        # Only groups that exist on this image: a server image has no\n"
        "        # lpadmin / plugdev, and useradd -G fails outright (rc=6) on a\n"
        "        # missing group.\n"
        "        ASB_GROUPS=\"\"\n"
        "        for g in sudo adm plugdev lpadmin dialout cdrom dip video; do\n"
        "            getent group \"$g\" >/dev/null 2>&1 && ASB_GROUPS=\"$ASB_GROUPS,$g\"\n"
        "        done\n"
        "        ASB_GROUPS=${ASB_GROUPS#,}\n"
        "        useradd -m -s /bin/bash -c 'AppSandbox User' ${ASB_GROUPS:+-G \"$ASB_GROUPS\"} \"$ASB_USER\" \\\n"
        "          && echo \"OK: useradd $ASB_USER (groups: $ASB_GROUPS)\" || echo \"FAIL: useradd rc=$?\"\n"
        "        if [ -n \"$ASB_HASH\" ]; then\n"
        ...
        "            asb-evict-simpledrm.service appsandbox-identity.service; do\n"
        ...
        "fi\n"
        "# VM identity overlay tool (applied by the agent / at boot when a\n"
        "# profile is set; see /opt/appsandbox/identity-README.md).\n"
        "if [ -f \"$EXTRAS/appsandbox-identity\" ]; then\n"
        "    install -m 0755 \"$EXTRAS/appsandbox-identity\" /usr/local/sbin/ \\\n"
        "      && echo \"OK: appsandbox-identity tool\" || echo \"FAIL: appsandbox-identity\"\n"
        "    [ -f \"$EXTRAS/identity-README.md\" ] && cp \"$EXTRAS/identity-README.md\" /opt/appsandbox/identity-README.md\n"
        "fi\n"
        "# Nested-KVM VPS replica tool (run by the user inside the guest; see\n"
        "# /opt/appsandbox/replica-README.md). Needs nested virt, which the VM has.\n"
        "if [ -f \"$EXTRAS/appsandbox-replica\" ]; then\n"
        "    install -m 0755 \"$EXTRAS/appsandbox-replica\" /usr/local/sbin/ \\\n"
        "      && echo \"OK: appsandbox-replica tool\" || echo \"FAIL: appsandbox-replica\"\n"
        "    [ -f \"$EXTRAS/replica-README.md\" ] && cp \"$EXTRAS/replica-README.md\" /opt/appsandbox/replica-README.md\n"
        "fi\n"
        "# Steam seats: several desktops (user + Xvnc + XFCE + Steam) on this\n"
        "# guest, no VM. Packages come at the first seat (see /opt/appsandbox/seat-README.md).\n"
        "if [ -f \"$EXTRAS/appsandbox-seat\" ]; then\n"
        "    install -m 0755 \"$EXTRAS/appsandbox-seat\" /usr/local/sbin/ \\\n"
        "      && echo \"OK: appsandbox-seat tool\" || echo \"FAIL: appsandbox-seat\"\n"
        "    [ -f \"$EXTRAS/seat-README.md\" ] && cp \"$EXTRAS/seat-README.md\" /opt/appsandbox/seat-README.md\n"
        "fi\n"
        "# Identity-patched QEMU for the replica: patches + build script only\n"
        "# (built on demand by 'appsandbox-replica qemu build').\n"
        "if [ -f \"$EXTRAS/qemu-identity/build.sh\" ]; then\n"
        "    chmod 0755 \"$EXTRAS/qemu-identity/build.sh\" && echo \"OK: qemu-identity sources\"\n"
        "fi\n"
        "# The product is Nestbox: the tools keep their appsandbox-* names (paths,\n"
        "# units and docs depend on them) and get nestbox-* aliases.\n"
        "for t in replica identity seat; do\n"
        "    [ -x /usr/local/sbin/appsandbox-$t ] && ln -sf /usr/local/sbin/appsandbox-$t /usr/local/sbin/nestbox-$t\n"
        "done\n"
        "# Snaps must not inherit the d3d12 selection: their own Mesa cannot\n"
        "# reach the WSL D3D12 runtime and ends up with no GL at all (App Center:\n"
        "# \"No GL implementation is available\"). Divert /usr/bin/snap to a\n"
        "# wrapper that strips the variables (survives snapd upgrades).\n"
        "if [ -f \"$EXTRAS/snap-no-gpu-env\" ] && [ -x /usr/bin/snap ]; then\n"
        "    if ! dpkg-divert --list /usr/bin/snap 2>/dev/null | grep -q snap.real; then\n"
        "        dpkg-divert --quiet --local --rename --divert /usr/bin/snap.real \\\n"
        "            --add /usr/bin/snap\n"
        "    fi\n"
        "    install -m 0755 \"$EXTRAS/snap-no-gpu-env\" /usr/bin/snap \\\n"
        "      && echo \"OK: snap wrapper (GPU env stripped for snaps)\" || echo \"FAIL: snap wrapper\"\n"
        "fi\n"
