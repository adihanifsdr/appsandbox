...
## Files
- `asb_ivshmem.h`  — shared IOCTL/GUID/struct contract (driver + user-mode clients).
- `asb_ivshmem.c`  — the KMDF function driver (binds VEN_1AF4&DEV_1110, maps BAR2 to user VA).
- `asb_ivshmem.inf` — install info; SUBSYS-qualified match outranks the inbox RAM-controller driver.
...
Produces `asb_ivshmem.sys`. (Finalize the exact project once the WDK layout is confirmed; set
...
2. `inf2cat /driver:. /os:10_NI_ARM64` (or appropriate) → `asb_ivshmem.cat`.
3. `signtool sign /v /fd sha256 /s My /n "AppSandbox Test" asb_ivshmem.sys asb_ivshmem.cat`.
4. **Enable test signing** (per project requirement — test-signed driver won't load otherwise):
   `bcdedit /set testsigning on`  → **reboot**.
5. Install: `pnputil /add-driver asb_ivshmem.inf /install` (rebinds the device from the inbox
